...
def ejecutar_cierre_nocturno_mora():
    ...
    print(
        f"🚀 [{fecha_hoy_str} 22:00] Iniciando proceso de cierre nocturno de mora..."
    )
    ...
    col_uuid = "UUID - Folio Fiscal"

    # Verificar que todas las columnas necesarias existan
    for col in [col_venc, col_monto, col_estatus, col_uuid]:
    ...
    # 3. Regla: Estatus "To be paid" y Vencimiento menor o igual a la fecha actual
    mask_vencidas = (
    ...
    print(f"📊 Facturas vencidas encontradas ('To be paid'): {len(vencidas)}")
    ...
    # 5. Guardar/Congelar facturas vencidas en el JSON usando UUID
    registros_actualizados = 0
    for idx, row in vencidas.iterrows():
        # Usamos el UUID como clave única
        factura_id = str(row[col_uuid]).strip()
    ...
    print(
        f"✅ Cierre nocturno completado. {registros_actualizados} facturas congeladas en el periodo '{periodo_actual}'."
    )
